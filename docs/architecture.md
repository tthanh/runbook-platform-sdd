...
| C-002 | Schema managed by EF Core migrations in `backend/src/RunbookPlatform.Api/Data/Migrations/` (the `EnsureCreated` mechanism of slice 001 has been retired) | adopted specs/003-runbook-execution; specs/004-rich-steps adds the additive `AddStepDetail` migration | Every schema change ships as an additive EF Core migration; no table is dropped, and migrations must apply cleanly over the existing store. `EnsureCreated` must never be mixed with migrations; the first migration of a schema-changing slice must reproduce the existing schema byte-for-byte so existing databases and tests are unaffected (constitution amendment 2026-06-15) |
...
| C-009 | Ordering by `DateTimeOffset` is done **in memory** after loading, never pushed to SQLite (`ORDER BY` on `DateTimeOffset` is unreliable on this provider); a per-Execution `Sequence` is the tiebreaker for records sharing a timestamp | slice-01 retro (confirmed) + specs/003-runbook-execution (mitigated); constitution amendment 2026-06-15 | Any slice that orders by `DateTimeOffset` must order in memory after loading; relying on SQLite to sort timestamps is a defect |
