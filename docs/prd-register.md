...
| 5 | 05-branching | Let a run take different paths instead of one straight line, so a procedure can encode "if X do A, else do B" instead of flattening every contingency into one list. | PRD drafted (approval pending) |
| 6 | 06-run-history-and-replay | Turn closed executions from write-only islands into a browsable, searchable history and let a reviewer replay a single run step by step — so the "how did we respond, and how do we respond better?" learning loop actually closes. | PRD drafted (approval pending) — build gated behind 05 |
...
| 05-branching | Finalized 2026-06-28 (Go) — full one-pager (new problem; #1 in the deferred ledger); appetite set to the minimal Decision-Flow model (named options → target step; coverage = taken path), cut-line is condition-expressions/loops/gates/parallel | RUN 2026-06-28 (compiled) — proposes the `Decision` Step Type (enum grows by one; Gate stays deferred) + routing terms (Option, Target step, Taken path); one candidate new event (Decision resolved); H1–H6 → ADRs/spikes at plan; language **ratified 2026-06-28** into the constitution (v1.1.0) — Decision/Option/Target Step/Taken Path + Computed Review coverage redefinition | — | — | — | — |
| 06-run-history-and-replay | Finalized 2026-07-01 (Go) — full one-pager (new problem; from the deferred "Around execution" cluster); appetite set to **medium** (browse/search past executions + single-run replay), cut-line is compare/diff, cross-run analytics/reporting, postmortem authoring; **build gated behind 05-branching's release** (replay must account for Decision events + taken path). Framing is capability-first: event-sourcing-vs-extend-the-append-only-log is left as an ADR-worthy mechanism decision, NOT mandated by appetite | RUN-narrow 2026-07-01 (compiled; reviewer pending) — workshop test all-no (no new binding language, no new events, one context) but run anyway as a **hotspot-harvesting** workshop since risk is paradigm-level. **No glossary amendments** (Run History / Replay stay capability words over existing terms — Execution, Step Record, Taken Path, Computed Review). Zero-new-write-side-events scope guard. Six hotspots routed to plan: H1 persistence mechanism (event store vs extend the append-only log — ADR, Principle I), H2 branched-run replay (05 dep — Taken Path + Decisions + NotReached), H3 bounded query/search surface (C-002/C-008), H4 deterministic replay ordering (reuse C-009), H5 read-UI stop line (C-003, no new deps), H6 cross-incident history with Incident external (C-008/ADR-0005) | — | — | — | — |
