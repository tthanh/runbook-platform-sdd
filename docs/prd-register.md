...
| 6 | 06-run-history-and-replay | Turn closed executions from write-only islands into a browsable, searchable history and let a reviewer replay a single run step by step — so the "how did we respond, and how do we respond better?" learning loop actually closes. | Discovery (Go) — build gated behind 05 |
